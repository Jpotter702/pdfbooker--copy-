...
1.0  **URL Input:** The system must allow the user to input a URL signifying the first webpage to begin scraping.
    
1.1  **Crawl Depth:** The system must allow the user to specify a number representing how many layers deep the crawler should go.
    
1.2  **Content Filters:** The system must provide options to keep or discard elements such as images, tables, code blocks, and stylistic formatting.
    
1.3  **Output Format Selection:** The system must allow users to choose between different output formats (text-only, text with images, or full webpage replica).
    
1.4  **Authentication:** The system should allow users to log in using a valid username and password, Google, or Github OAuth.
    
1.5  **User Dashboard:** The system should provide a dashboard where users can view and manage their previously created PDF books.
    
1.6  **Template Selection:** The system should offer multiple pre-designed templates for PDF formatting.
    
1.7  **Custom Styling:** The system should allow users to customize fonts, colors, and layout of the generated PDF.
    
1.8  **Progress Indicator:** The system must display a progress indicator during the scraping and PDF generation process.
    
1.9  **PDF Preview:** The system should provide a preview of the generated PDF before final download.
